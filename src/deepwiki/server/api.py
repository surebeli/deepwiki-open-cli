...
def create_app(cors_origins: list[str] | None = None) -> FastAPI:
    ...
    def _get_cache_path(repo: str, language: str) -> Path:  # pragma: no cover
        from deepwiki.data.repo_manager import _default_repo_cache_dir
    ...
    def get_wiki_cache(repo: str, owner: str | None = None, language: str = "en") -> dict[str, object]:  # pragma: no cover
        try:
            lookup_repo = repo
    ...
    async def create_wiki_cache(request: dict) -> dict[str, object]:  # pragma: no cover
        try:
    ...
    def delete_wiki_cache(repo: str, language: str = "en") -> dict[str, object]:  # pragma: no cover
        try:
            cache_path = _get_cache_path(repo, language)
            if cache_path.exists():
                cache_path.unlink()
    ...
    @app.get("/api/models/config")
    def models_config() -> dict[str, object]:  # pragma: no cover
    ...
    async def chat_stream_api(request: AskRequest):  # pragma: no cover
        from fastapi.responses import StreamingResponse
        return await chat_stream_internal(request)

    @app.post("/chat/completions/stream")
    async def chat_stream_completions(request: AskRequest):  # pragma: no cover
        return await chat_stream_internal(request)

    async def chat_stream_internal(request: AskRequest):  # pragma: no cover
        from fastapi.responses import StreamingResponse
        import re
        try:
    ...
    async def websocket_chat(websocket: WebSocket):  # pragma: no cover
        import re
